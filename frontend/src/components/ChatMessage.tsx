  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-3 sm:mb-4`}>
      ...
      >
        ...
        {!isUser && snippet && (
          <div className="mt-3 rounded-lg border-l-4 border-amber-400 bg-amber-50 p-2 sm:p-3 text-xs sm:text-sm text-gray-800 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-amber-700">Primary match</p>
            <p className="mt-2 whitespace-pre-line leading-relaxed break-words">{snippet}</p>
          </div>
        )}
          ...
          <div className="mt-3 space-y-2 text-xs sm:text-sm text-gray-600">
            {extraSnippets.map((text, index) => (
              <div
                ...
                className="rounded-md border border-gray-200 bg-white p-2 sm:p-3 text-gray-700 shadow-sm"
              >
                <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Supporting snippet</p>
                <p className="mt-1 whitespace-pre-line leading-relaxed break-words">{text}</p>
              </div>
            ))}
          </div>
        )}
